...
class Usuario:
    ...
    def validar_registro(data):
        ...
        if not EMAIL_REGEX.match( data['email'] ):
            es_valido = False 
            flash( "Por favor proporciona un email válido (solo letras).", "error_email")   
        
        if len( data['password'] ) < 8:
            es_valido = False
            flash( "Tu contraseña necesita tener al menos 8 caracteres.", "error_password")
        
        if 'confirmacion_password' in data and data['password'] != data['confirmacion_password']:
            es_valido = False
            flash( "Tu contraseñas no coinciden.", "error_password")
